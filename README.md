...
`LazyImport[T]` is a generic class. Annotating a variable with the resolved type gives you full type-checking and autocompletion downstream — the type checker knows that calling or accessing the proxy produces a `T`.
...
ndarray: LazyImport[np.ndarray] = lazy_import("numpy", "ndarray")
arr = ndarray([1, 2, 3])  # arr: np.ndarray

# type[DataFrame] means calling the proxy produces a DataFrame instance
DataFrame: LazyImport[type[DataFrame]] = lazy_import("pandas", "DataFrame")
...
MkPath: LazyImport[type[Path]] = lazy_import("pathlib", "Path")
...
### `lazy_import(module_name, *attrs) -> LazyImport[T]`

Create a lazy proxy for `module_name`. Optional `attrs` are an attribute chain traversed after import. The function is generic — annotate the variable as `LazyImport[T]` to propagate the resolved type to callers.
...
The generic proxy class returned by `lazy_import`. Annotate the variable with `LazyImport[T]` to tell the type checker what type the proxy resolves to — `__call__` and `_resolve()` are typed to return `T`. Supports call, attribute access, `dir()`, and `repr()`. Thread-safe.
